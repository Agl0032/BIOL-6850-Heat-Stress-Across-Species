...
## Uses the "fastqc_data.txt" files generated by FastQC to calculate the average sequence quality
## for each sample. Also extracts count of sequences. Assumes the presence of 2 subfolders containing
## FastQC outputs for the same samples. Only considers paired sequences for cleaned data.
...
## Outputs a table of values in a file called "quality-table.csv", which can be changed by changing
